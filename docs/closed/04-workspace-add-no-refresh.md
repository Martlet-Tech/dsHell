...
**收口时间**：2026-09-14 · **状态**：已修好并随 `v0.1.2` 交付。

### 10.1 最终解法

不在壳层"把焦点还回去"，也不是强制重绘，而是**换掉弹框的人**：

| | 谁弹框 | parent | 结果 |
| --- | --- | --- | --- |
| dsh 的 `-native` 后端 | 独立 node 子进程 | `Show(null)`，**无主** | 抢前台 → 主窗口失焦 → WebView2 挂起渲染进程 |
| DShell 壳层（最终） | Rust 进程自己 | `set_parent(&window)`，**有主** | 主窗口不失焦，挂起的前提不存在 |

有主模态框不会让父窗口失焦，所以 `CalculateNativeWinOcclusion` 那条遮挡判定
根本不会被触发 —— 这是**消除前提**，而不是**事后补救**。

两端用 loopback HTTP 接通：壳层起一个只监听 `127.0.0.1` 的端点（共享令牌鉴权），
dsh 侧的插件把选目录请求转过来。端口与令牌经环境变量 `DSHELL_PICKER_PORT` /
`DSHELL_PICKER_TOKEN` 交给 dsh。
...
| 立项时预留 | 实际结论 |
| --- | --- |
| 根因 H1（遮挡节流）为主 | ✅ 成立，但**主因的准确表述**是"无主窗口抢前台"，遮挡节流只是它触发的下游机制 |
| H3（焦点未归还）待区分 | ❌ **不是独立成因**。有了 owner 之后不存在失焦，无需再归还 |
| 修法是 `src-tauri/` 内侧一行 | ❌ 实际远不止一行：新增 `picker.rs`（371 行）+ 一个 dsh 插件（239 行）+ 自动安装（`plugin.rs`） |
| `initialization_script` 不生效 | ✅ 成立且**已被后续推翻式验证**：R17–R19 证明 `initialization_script` 与 `eval` **两条注入通道都是死的**，详见 `05` §19 |

> 最后一条是本次立项最有价值的副产物：它否定了"往 DSH 页面注入"这条整条路线，
> 把方案逼到 dsh 官方插件机制上。该结论已写入 `design.md`。

### 10.3 实测结果

| 验证项 | 结果 | 测量时间与环境 |
| --- | --- | --- |
| 对话框有主（日志 `parented to the main window`） | ✅ | 2026-09-14，本机 |
| 端到端：点「添加工作区」→ 不失焦、不挂起、**自动刷新** | ✅ 用户实测，无需补点 | 2026-09-14 |
| 选目录耗时 | 2328 / 1775 / 2132 / **1303** ms | 2026-09-14 |
| 对照：修好前的同类操作 | 6430 / 7453 ms | 2026-09-14 早 |
| 无 `DSHELL_PICKER_PORT` 时不影响内置 picker | ✅ | 2026-09-13 |
| 令牌校验（无令牌 403） | ✅ | 2026-09-13 |

### 10.4 交付方式的演变（收口后的追加改动）

初版要求用户手动跑 `scripts/install-picker-plugin.ps1` 把插件装进 dsh profile。
这个要求在 2026-09-14 被取消：**exe 启动体检时自己装**（第 ⑥ 项）。

理由是这个手动步骤的失败方式**极其隐蔽**——漏装不报错、不崩溃，只是静默退回
"选完要补点一下"。而这恰恰是本立项要修的症状，等于没修。详见 `05` §19.8。

| | 初版（`v0.1.1` 之前） | 收口版（`v0.1.2`） |
| --- | --- | --- |
| 插件来源 | 用户手动装 | 随 exe 发布，启动时自动装 |
| 需要 pnpm | 手动路线不需要，但官方 `dsh plugin` 命令需要 | **不需要**（纯文件复制） |
| 漏装后果 | 静默降级 | 体检黄字提示 + CI 拦住残包 |

### 10.5 遗留

| # | 项 | 说明 |
| --- | --- | --- |
| L1 | 发布物是"exe + `plugin\` 目录"两个东西 | 根因：`bundle.active: false` 时 Tauri 不把 resource 嵌进 exe。冒烟测试与 release 流程都已强制两者同行，但**用户可能只拖走 exe** |
| L2 | 只装 `web` profile | DShell 只启动 `dsh web`，够用；若将来支持 headless 需扩展 |
| L3 | `set_parent` 与"不挂起"的因果仍是强旁证 | 未做"只改 parent"的对照实验（DSH Desktop 用同一机制且无此问题） |
| L4 | 未验证 WebView2 完全缺失的兜底 | 与本立项无关，见 `roadmap.md` §6 |
